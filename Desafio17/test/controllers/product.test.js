...
test('[GET] product/:non-existent-id returns product not found', async() => {
    const nonExistentProductUrl = 'http://localhost:8080/products/non-existent-id';
...
test('[PUT] product/:non-existent-id returns product not found or invalid body content', async() => {
    const nonExistentProductUrl = 'http://localhost:8080/products/non-existent-id';
    const body = {}
